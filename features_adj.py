""" Convert a list of book titles to a word2vec representation and compute surprise value for adjucent pairs"""
...
def surprise(pairs,probs):
    ...
    surprise = []
    for title in pairs:
        list = [probs[b][a] for (a,b) in title]
...
def main():
    ...
    for k, (i,prob) in enumerate(surp):
        ...
        word = title[indices[k][i]]
        next_word = title[indices[k][i+1]]
